...
type
  ...
  TCheckAttribute = (caText,caComment,caString,caDocumentation,caIdentifier, caSymbol, caKey);
  TCheckAttributes = set of TCheckAttribute;

const
  AllAttributes = [caText,caComment,caString,caDocumentation,caIdentifier, caSymbol, caKey];
...
type
  ...
  public
    ...
    procedure IgnoreDictWord (const AWord : String);
    ...
    property UserIgnoreDict : TStringList read FUserIgnoreDict;
  published
    { Properties }
...
// User dictionary
  ...
  FUserIgnoreDict := TStringList.Create(TDuplicates.dupIgnore, true, true);
...
// List of highlighter attributes to be checked
  with FCheckAttribs do begin
    ...
    if caIdentifier in ACheckAttri then Add('Identifier');
    if caSymbol in ACheckAttri then Add('Symbol');
    if caKey in ACheckAttri then Add('Key');
    end;
  end;
...
procedure TSynEditSpellCheck.IgnoreDictWord(const AWord: String);
{Unlike Add to User dictionary this will ignore the word during the session}
var
  sw : String;
begin
  sw:=Trim(AWord);
  if FEnabled and Assigned(FHunspellHandle) and (length(sw)>0) then
  with FUserIgnoreDict do
    if IndexOf(sw)<0 then
    begin
      Add(sw);
...
procedure TSynEditSpellCheck.ClearIgnore;
begin
  FUserIgnoreDict.Clear;
end;
